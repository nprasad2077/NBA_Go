...
func importGameSchedules(db *gorm.DB) {
	months := []string{"september", "october"}

	for season := 2005; season <= 2005; season++ {
...
func importBoxScores(db *gorm.DB) {
	from := time.Date(2005, time.September, 26, 0, 0, 0, 0, time.UTC)
	to := time.Date(2005, time.October, 16, 5, 30, 0, 0, time.UTC)
