...
func importGameSchedules(db *gorm.DB) {
	months := []string{"october", "november"}

	for season := 2003; season <= 2003; season++ {
...
func importBoxScores(db *gorm.DB) {
	from := time.Date(2003, time.October, 30, 0, 0, 0, 0, time.UTC)
	to := time.Date(2003, time.November, 19, 5, 30, 0, 0, time.UTC)
